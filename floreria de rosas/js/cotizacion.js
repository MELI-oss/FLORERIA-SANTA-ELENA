...
formItem.addEventListener('submit', (e) => {
    ...
    const presentacion = inputPresentacion.value;
...
function actualizarInterfaz() {
    ...
    if (items.length === 0) {
        tablaBody.innerHTML = `
            <tr id="fila-vacia">
                <td colspan="6" class="texto-vacio">No hay elementos agregados aún.</td>
            </tr>
        `;
    ...
    items.forEach((item) => {
        ...
        tr.innerHTML = `
            ...
            <td class="col-presentacion">${escapeHTML(item.presentacion || 'Unidad')}</td>
